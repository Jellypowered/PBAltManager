...
function PBAM.IsRosterSortVisible(mode, tabName)
    ...
    tabName = tostring(tabName or PBAM.CurrentTab or "")
    if mode == "profession" then return tabName == "Professions" or tabName == "Trainer" or tabName == "Inventory" end
...
function PBAM.RegisterCoreBridgeCallbacks()
    ...
    PBAM.Bridge.RegisterCallback("BotSkillsUpdated", function(botName)  
        ...
        if (PBAM.CurrentTab == "Professions" or PBAM.CurrentTab == "Trainer" or PBAM.CurrentTab == "Inventory") and PBAM.GetActiveRosterSortMode and PBAM.GetActiveRosterSortMode() == "profession" then
            PBAM.QueueRosterRefresh()
        end
    end)
...
function PBAM.CreateMainWindow()
    ...
    botListFrame:SetScript("OnMouseWheel", function(self, delta)
        ...
        -- Increase scroll speed when in a profession-sorted roster view (more rows to scroll)
    ...
    local statusBox = CreateFrame("Frame", nil, tabBar)
    statusBox:SetPoint("TOPLEFT", tabBar, "TOPLEFT", 10, -2)
    statusBox:SetPoint("BOTTOMRIGHT", tabBar, "BOTTOMRIGHT", -352, 2)
    statusBox:SetFrameLevel(tabBar:GetFrameLevel() + 1)
    statusBox:EnableMouse(false)

    local mainStatus = PBAM.CreateStatusText(statusBox)
    mainStatus:SetPoint("LEFT", statusBox, "LEFT", 8, 0)
    mainStatus:SetPoint("RIGHT", statusBox, "RIGHT", -8, 0)
    mainStatus:SetHeight(24)
    mainStatus:SetJustifyH("LEFT")
    mainStatus:SetJustifyV("MIDDLE")
    mainStatus:SetTextColor(0.08, 0.08, 0.08, 1)
    PBAM.WrapFontString(mainStatus, 700)
    ...
    PBAM.TabBar = tabBar
    PBAM.MainStatusBox = statusBox
    PBAM.MainStatusText = mainStatus
...
function PBAM.SwitchTab(tabName)
    ...
    if tab.panel then
        tab.panel:Show()
        if tab.panel._sharedStatusDefault and PBAM.SetStatusText and PBAM.MainStatusText then
            PBAM.SetStatusText(PBAM.MainStatusText, tab.panel._sharedStatusDefault.msg or "", tab.panel._sharedStatusDefault.kind or "info")
        end
    end
...
function PBAM.RefreshRosterDisplay()
    ...
    elseif sortMode == "profession" and (PBAM.CurrentTab == "Professions" or PBAM.CurrentTab == "Trainer" or PBAM.CurrentTab == "Inventory") and PBAM.Bridge then
