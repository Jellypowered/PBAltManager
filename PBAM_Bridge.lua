...
function Bridge.RunInventoryItemAction(bot, action, itemId, count, bag, slot)
    local t=makeToken("item")
    action = string.upper(trim(action or ""))
    Bridge.InventoryItemActions[t] = {
        botName = bot,
        action = action,
...
function Bridge.OnAddonMessage(prefix, message, channel, sender)
    ...
    elseif opcode == "GBANK" or opcode == "GBANK_BEGIN" or opcode == "GBANK_RIGHTS" or opcode == "GBANK_SUMMARY" or opcode == "GBANK_ITEM" or opcode == "GBANK_ERROR" or opcode == "GBANK_END" then
...
function Bridge.ApplyGuildBankPayload(payload)
    ...
    if opcode == "GBANK_BEGIN" then
        Bridge.GuildBank[key] = { name = name, token = token, items = {}, loading = true, canWithdraw = false, withdrawRemaining = 0 }
        return
    end

    local gbank = Bridge.GuildBank[key]
    if not gbank or gbank.token ~= token then
        Bridge.DebugPrint("[GBANK DEBUG] Ignoring stale guild bank chunk opcode=" .. tostring(opcode) .. " bot=" .. tostring(name) .. " token=" .. tostring(token) .. " current=" .. tostring(gbank and gbank.token or "nil"))
        return
    end

    if opcode == "GBANK_RIGHTS" then
        local canWithdraw, withdrawRemaining = splitOnce(rest, "~")
        gbank.canWithdraw = trim(canWithdraw) == "1"
        gbank.withdrawRemaining = tonumber(withdrawRemaining) or 0
        Bridge.GuildBank[key] = gbank
    elseif opcode == "GBANK_SUMMARY" then
        gbank.numSlots = tonumber(rest) or 0
        Bridge.GuildBank[key] = gbank
    elseif opcode == "GBANK_ITEM" then
        gbank.items = gbank.items or {}
        local itemText = trim(urlDecode(rest))
        if itemText ~= "" then table.insert(gbank.items, { text = itemText, itemId = parseItemLinkId(itemText) }) end
        Bridge.GuildBank[key] = gbank
    elseif opcode == "GBANK_ERROR" then
        gbank.error = trim(urlDecode(rest))
        Bridge.GuildBank[key] = gbank
    elseif opcode == "GBANK_END" then
        gbank.loading = false
        Bridge.GuildBank[key] = gbank
        Bridge._InFlightRequests["GBANK~" .. key] = nil
        Bridge.FireCallback("GuildBankUpdated", name, gbank)
    end
end
